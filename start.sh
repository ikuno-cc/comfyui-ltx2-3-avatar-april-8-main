...
download_if_missing() {
  ...
  for i in "${!backoffs[@]}"; do
    set +e
    output=$(HF_TOKEN="${HF_TOKEN:-}" comfy model download \
      --url "$url" \
    ...
    if echo "$output" | grep -q "No space left on device"; then
      echo "[ERROR] Disk full while downloading $filename" >&2
      exit 1
    fi
...
download_if_missing \
  "https://huggingface.co/AviadDahan/LTX-2.3-ID-LoRA-TalkVid-3K/resolve/main/lora_weights.safetensors" \
  "models/loras" \
  "ltx-2.3-id-lora-talkvid-3k.safetensors"

download_if_missing \
  "https://huggingface.co/rahul7star/Ltx-2-3-Lora-Collection/resolve/f79a7ffc9a0868739405fe3416698088afab0f7d/LTX2.3_Reasoning_V1.safetensors" \
  "models/loras" \
